...
Function LoadGame(file$)
	...
	If ReadInt(f) <> 113 Then RuntimeError("无法加载游戏，存档文件已损坏（错误2.5）")
	...
	For i = 1 To temp
		...
		DebugLog("正在加载NPC " +n\NVName+ " (ID "+n\ID+")")
	...
	If ReadInt(f) <> 632 Then RuntimeError("无法加载游戏，存档文件已损坏（错误1）")
...
;        EndIf
	...
	If ReadInt(f) <> 954 Then RuntimeError("无法加载游戏，存档文件已损坏（错误2）")
	...
	If ReadInt(f) <> 1845 Then RuntimeError("无法加载游戏，存档文件已损坏（错误3）")
...
Function LoadGameQuick(file$)
	...
	If ReadInt(f) <> 954 Then RuntimeError("无法加载游戏，存档文件可能已损坏（错误2）")
...
Function LoadSaveGames()
	...
	SaveGameAmount = 0
	If FileType(SavePath)=1 Then RuntimeError "无法创建文件夹 "+Chr(34)+SavePath+Chr(34)
