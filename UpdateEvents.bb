								...
								Else
									...
									If (EntityDistance(Collider, e\room\NPC[3]\Collider)<1.5) Then
										If EntityInView(e\room\NPC[3]\obj, Camera) Then
											...
											If MouseHit1 Then
												SelectedItem = CreateItem("SCP-173文档", "paper", 0.0, 0.0, 0.0)
							...
							If EntityInView(e\room\Objects[0], Camera) Then
								...
								If MouseHit1 Then
									...
									ElseIf e\EventState2 = 1 And (Not inserted) Then
										Using294=False
										Msg = "You need to insert another 硬币 in order to use this machine."
										MsgTimer = 70*5
									ElseIf (Not inserted) Then
										Using294=False
										Msg = "You need to insert two 硬币s in order to use this machine."
										MsgTimer = 70*5
									EndIf
								EndIf
							EndIf
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-FPSfactor =< 70*7.5 Then
								...
								;钱包 spawning (with 3 coins)
								it.Items = CreateItem("钱包","wallet",EntityX(e\room\NPC[0]\Collider,True),EntityY(e\room\NPC[0]\Collider,True),EntityZ(e\room\NPC[0]\Collider,True))
								...
								For i = 0 To 1
									it2.Items = CreateItem("硬币","25ct",1,1,1)
									it2\Picked = True
									it2\Dropped = -1
								...
								If e\room\grid\grid[ix+(iy*gridsz)]>0 Then
									...
									Select e\room\grid\grid[ix+(iy*gridsz)]
										...
										Case 7
											...
											it = CreateItem("夜视仪", "nvgoggles",e\room\x+ix*2.0-(Sin(EntityYaw(tempInt,True))*504.0*RoomScale)+(Cos(EntityYaw(tempInt,True))*16.0*RoomScale), 8.0+(80.0*RoomScale), e\room\z+iy*2.0+(Cos(EntityYaw(tempInt,True))*504.0*RoomScale)+(Sin(EntityYaw(tempInt,True))*16.0*RoomScale))
											EntityType (it\collider, HIT_ITEM)
									End Select
							...
							If (EntityDistance(Collider, e\room\Objects[2])<1.5) Then
								...
								If MouseHit1 Then
									If ItemAmount >= MaxItemAmount Then
										...
										MsgTimer = 70 * 5
									Else
										SelectedItem = CreateItem("绘画", "paper", 0.0, 0.0, 0.0)
										EntityType SelectedItem\collider,HIT_ITEM
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 18
								...
								it.Items = CreateItem("奇怪的笔记", "paper", TFormedX(), TFormedY(), TFormedZ())
								EntityType(it\collider, HIT_ITEM)
							Case 25
							...
							Case 30
								...
								If Inventory(i)<>Null Then RemoveItem(Inventory(i))
								Inventory(i) = CreateItem("奇怪的笔记", "paper", 1,1,1)
								HideEntity Inventory(i)\collider
								Inventory(i)\Picked = True
							Case 35
								For i = 0 To 3
					...
					Else
						...
						If e\EventState = 0 Then
							...
							If EntityDistance(e\room\Objects[11],Collider)<EntityDistance(e\room\Objects[12],Collider) Then
								it = CreateItem("研究部门-02设计方案", "paper", EntityX(e\room\Objects[11],True),EntityY(e\room\Objects[11],True),EntityZ(e\room\Objects[11],True))
								EntityType it\collider,HIT_ITEM
							Else
								it = CreateItem("研究部门-02设计方案", "paper", EntityX(e\room\Objects[12],True),EntityY(e\room\Objects[12],True),EntityZ(e\room\Objects[12],True))
								EntityType it\collider,HIT_ITEM
							EndIf
						ElseIf e\EventState > 0
			...
			Case "room1162"
				...
				If PlayerRoom = e\room
								...
								If Rand(8)=1 Then
									...
									Select (e\EventState)
										Case 1
											itemName = "丢失的钥匙"
										Case 2
											itemName = "处罚听证会记录DH-S-4137-17092"
										Case 3
											itemName = "硬币"
										Case 4
											itemName = "电影票"
					...
					ElseIf e\EventState3 >= 3.0
						...
						Select e\EventState
							Case 1
								it = CreateItem("丢失的钥匙","key",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 2
								it = CreateItem("处罚听证会记录DH-S-4137-17092","oldpaper",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 3
								it = CreateItem("硬币","coin",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 4
								it = CreateItem("电影票","ticket",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
							Case 5
								it = CreateItem("破旧的身份证","badge",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True))
						End Select
						EntityType(it\collider, HIT_ITEM)
						GiveAchievement(Achv1162)
						MouseHit1 = False
