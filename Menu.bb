...
Function UpdateMainMenu()
	...
	Else
		...
		Select MainMenuTab
			Case 1 ; New game
				...
				Color(255, 255, 255)
				AASetFont Font2
				AAText(x + width / 2, y + height / 2, "新游戏", True, True)
				...
				Else
					AAText (x + 20 * MenuScale, y + 60 * MenuScale, "已选地图:")
					Color (255, 255, 255)
				...
				If SelectedDifficulty\customizable Then
					...
					AAText(x + 200 * MenuScale, y + 165 * MenuScale, "永久死亡")
					...
					Select SelectedDifficulty\otherFactors
						Case EASY
							AAText(x + 200 * MenuScale, y + 255 * MenuScale, "其它难度预设: 简单")
						Case NORMAL
							AAText(x + 200 * MenuScale, y + 255 * MenuScale, "其它难度预设: 普通")
						Case HARD
							AAText(x + 200 * MenuScale, y + 255 * MenuScale, "其它难度预设: 困难")
					End Select
				Else
			...
			Case 2 ;load game
				...
				AAText(x+(width/2.0),y+536*MenuScale,"第 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SaveGameAmount)/6.0))),1))+" 页",True,True)
				...
				If SaveGameAmount = 0 Then
					AAText (x + 20 * MenuScale, y + 20 * MenuScale, "没有保存的存档。")
				Else
					x = x + 20 * MenuScale
					y = y + 20 * MenuScale
			...
			Case 3,5,6,7 ;options
				...
				If MainMenuTab = 3 ;Graphics
					...
					Color 255,255,255
					AAText(x + 20 * MenuScale, y, "启用房间光亮:")
					...
					Color 255,255,255
					AAText(x + 20 * MenuScale, y, "纹理细节层次:")
					...
					Color 255,255,255
					AAText(x + 20 * MenuScale, y, "在VRAM中存储贴图:")
				...
				ElseIf MainMenuTab = 5 ;Audio
					...
					Color 255,255,255
					AAText x + 20 * MenuScale, y, "自动释放音频:"
					...
					Color 255,255,255
					AAText x + 20 * MenuScale, y, "启用自定频道:"
					...
					If EnableUserTracks
						y = y + 30 * MenuScale
						Color 255,255,255
						AAText x + 20 * MenuScale, y, "自定频道模式:"
						UserTrackMode = DrawTick(x + 310 * MenuScale, y + MenuScale, UserTrackMode)
						If UserTrackMode
							AAText x + 350 * MenuScale, y + MenuScale, "循环"
						Else
							AAText x + 350 * MenuScale, y + MenuScale, "随机"
						EndIf
						If MouseOn(x+310*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
							DrawOptionsTooltip(tx,ty,tw,th,"usertrackmode")
						EndIf
						If DrawButton(x + 20 * MenuScale, y + 30 * MenuScale, 190 * MenuScale, 25 * MenuScale, "扫描自定频道",False)
							DebugLog "已开启自定频道检查"
						...
						EndIf
						If UserTrackCheck%>0
							AAText x + 20 * MenuScale, y + 100 * MenuScale, "自定频道已创建 ("+UserTrackCheck2+"/"+UserTrackCheck+" 个文件已加载)"
						EndIf
					Else
						UserTrackCheck%=0
					EndIf
				...
				ElseIf MainMenuTab = 6 ;Controls
					...
					Color(255, 255, 255)
					AAText(x + 20 * MenuScale, y + 2, "鼠标灵敏度：")
					If MouseOn(x+310*MenuScale,y-4*MenuScale,150*MenuScale+14,20)
					...
					Color(255, 255, 255)
					AAText(x + 20 * MenuScale, y, "反转鼠标Y轴:")
				...
				ElseIf MainMenuTab = 7 ;Advanced
					...
					Color 255,255,255
					AAText(x + 20 * MenuScale, y, "出现错误时打开控制台:")
			...
			Case 4 ; load map
				...
				Color(255, 255, 255)
				AASetFont Font2
				AAText(x + width / 2, y + height / 2, "加载地图", True, True)
				...
				AAText(x+(width/2.0),y+536*MenuScale,"第 "+Int(Max((CurrLoadGamePage+1),1))+"/"+Int(Max((Int(Ceil(Float(SavedMapsAmount)/6.0))),1))+" 页",True,True)
				...
				If SavedMaps(0)="" Then 
					AAText (x + 20 * MenuScale, y + 20 * MenuScale, "没有保存的地图。使用地图制作器创建地图。")
				Else
					x = x + 20 * MenuScale
					y = y + 20 * MenuScale
					For i = (1+(6*CurrLoadGamePage)) To 6+(6*CurrLoadGamePage)
	...
	AAText 20,GraphicHeight-45,"v"+VersionNumber
	AAText 20,GraphicHeight-30,SinicizationNumber
	
	TextSetColor 255,255,0
	AASetFont Font1
	TextDraw GraphicWidth-20,10,"汉化制作:子悦汉化组",TEXT_RIGHT,50,encoding
	TextDraw GraphicWidth-20,TextFontWidth()+11,"对外交流群1006841985",TEXT_RIGHT,TEXT_BOTTOM,encoding
...
Function UpdateLauncher()
	...
	Font1 = TextLoadFont("Courier New", 18, False, False, False, TEXT_DEFAULT, "GFX\font\cour\Courier New.ttf")
	SetFont Font1
	MenuWhite = LoadImage_Strict("GFX\menu\menuwhite.jpg")
	MenuBlack = LoadImage_Strict("GFX\menu\menublack.jpg")	
	MaskImage MenuBlack, 255,255,0
	...
	Repeat
		InitAAFont()
		Font1 = TextLoadFont("Containment Breach", 23, False, False, False, TEXT_DEFAULT, "GFX\font\cour\Containment Breach.ttf")
		SetFont Font1
		;Cls
		Color 0,0,0
		...
		Text(40 + 430 + 15, 262 - 55 + 5 - 12, "全屏")
		Color 255, 255, 255
		Text(40 + 430 + 15, 262 - 55 + 43 - 11, "无边窗口")
		...
		Color 255, 255, 255
		Text(40 + 430 + 15, 262 - 55 + 95 + 4, "使用启动器")
		
		If (Not BorderlessWindowed)
			If Fullscreen
				Text(40+ 260 + 15, 262 - 55 + 140 - 6, "当前分辨率: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + "," + (16+(16*(Not Bit16Mode)))))
			Else
				Text(40+ 260 + 15, 262 - 55 + 140 - 6, "当前分辨率: "+(GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + ",32"))
			EndIf
		Else
			Text(40+ 260 + 15, 262 - 55 + 140 - 6, "当前分辨率: "+GfxModeWidths(SelectedGFXMode) + "x" + GfxModeHeights(SelectedGFXMode) + ",32")
			If GfxModeWidths(SelectedGFXMode)<G_viewport_width Then
				Text(40+ 260 + 65, 262 - 55 + 160, "(分辨率应升至")
			...
			ElseIf GfxModeWidths(SelectedGFXMode)>G_viewport_width Then
				Text(40+ 260 + 65, 262 - 55 + 160, "(分辨率应降至")
				Text(40+ 260 + 65, 262 - 55 + 180, G_viewport_width + "x" + G_viewport_height + ",32)")
			EndIf
		EndIf
		...
		If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 50 - 55, 100, 30, "启动", False, False, False) Then
		...
		If DrawButton(LauncherWidth - 30 - 90, LauncherHeight - 50, 100, 30, "退出", False, False, False) Then End
		Flip
	Forever
	...
	;EndIf
	TextDeinitialize() ;取消初始化并删除其所有资源
End Function
...
Function DrawLoading(percent%, shortloading=False)
	...
	Repeat 
		...
		If SelectedLoadingScreen\title = "CWM" Then
			...
			Next		
			AASetFont Font1
			RowText2(strtemp, GraphicWidth / 2-200, GraphicHeight / 2 +120,400,300,True)		
		Else
			...
			AASetFont Font1
			TextDrawRect GraphicWidth / 2-200+1, GraphicHeight / 2 +120+1,400,300,SelectedLoadingScreen\txt[LoadingScreenText],TEXT_CENTER, TEXT_WORDWRAP Or TEXT_DONTCLIP, encoding
			
			Color 255,255,255
			AASetFont Font2
			AAText(GraphicWidth / 2, GraphicHeight / 2 +80, SelectedLoadingScreen\title, True, True)
			AASetFont Font1
			TextDrawRect GraphicWidth / 2-200, GraphicHeight / 2 +120,400,300,SelectedLoadingScreen\txt[LoadingScreenText],TEXT_CENTER, TEXT_WORDWRAP Or TEXT_DONTCLIP, encoding
		...
		If percent = 100 Then 
			...
			AAText(GraphicWidth / 2, GraphicHeight - 50, "按任意键继续", True, True)
		Else
			FlushKeys()
			FlushMouse()
		EndIf
...
Function DrawButton%(x%, y%, width%, height%, txt$, bigfont% = True, waitForMouseUp%=False, usingAA%=True)
	...
	If usingAA Then
		...
		AAText(x + width / 2, y + height / 2 - 1, txt, True, True)
	Else
...
Function RowText(A$, X, Y, W, H, align% = 0, Leading#=1)
	TextDrawRect X, LinesShown * Height + Y,W,H, A$, TEXT_LEFT, TEXT_WORDWRAP Or TEXT_DONTCLIP, encoding
End Function

Function RowText2(A$, X, Y, W, H, align% = 0, Leading#=1)
	TextDrawRect X,Y,W,H, A$, TEXT_CENTER, TEXT_WORDWRAP Or TEXT_DONTCLIP, encoding
End Function
