...
AppTitle "转换B3D文件（3D World Studio生成的文件）至RMESH"
...
Color 255,255,255
Text 5,5,"按下一个按键："
Text 5,25,"1 - 转换B3D文件（3D World Studio生成的文件）至RMESH"
Text 5,45,"2 - 转换指定的B3D文件（3D World Studio生成的文件）至RMESH"
Text 5,65,"3 - 重置rooms.ini以使用B3D meshes文件"
Text 5,85,"ESC - 直接退出"
Flip
	...
	While Not Eof(f)
		...
		If Left(TemporaryString,1) = "[" Then
			...
			If TemporaryString <> "room ambience" Then
				...
				Cls
				Text 5,5,"已转换： "+Chr(34)+Stri+Chr(34)
				Flip
...
Else If state=2
	...
	Flip
	FlushKeys()
	Text 0,0,"需要转换的文件的路径：" ;这个真的很奇怪，FastText和SpeedText都找不到对应的解决方方案
	Stri=Input("                     ") ;所以这里把显示文字和输入系统分开，可以正常显示中文，也可以正常输入路径
	mesh=LoadAnimMesh(Stri)
	SaveRoomMesh(mesh,Replace(Stri,".b3d",".rmesh"))
	Cls
	Text 5,5,Stri+" 转换完毕"
...
;~F#2E#33#19F#1AD#1B1#202#210#218#220#224
;~C#Blitz3D