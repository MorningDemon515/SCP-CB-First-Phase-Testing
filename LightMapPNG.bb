...
Color 255,255,255
Text 5,5,"按下一个按键："
Text 5,25,"1 - 将BMP转换为PNG文件，并修改RMESH以使用它们"
Text 5,45,"2 - 将特定房间的BMP转换为PNG文件，并对其进行修改以使用它们"
Text 5,65,"3 - 重置options.ini以使用旧的RMESH"
Text 5,85,"ESC - 直接退出"
Flip
	...
	While Not Eof(f)
		...
		If Left(TemporaryString,1) = "[" Then
			...
			If TemporaryString <> "room ambience" Then
				...
				Cls
				Text 5,5,Chr(34)+Stri+Chr(34)+" 已转换"
				Flip
...
ElseIf state=2
	...
	Flip
	FlushKeys()
	Text 0,0,"需要转换的房间文件路径： "
	Stri=Input("                        ")
	LoadRMesh(Stri)
	Cls
	Text 5,5,"文件 "+Stri+" 转换完毕"
	Flip
	Delay 1000
