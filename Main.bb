...
;	 注：请不要直接在IDE（例如IDEal和Blitz3D自带IDE）内修改（编码会被改为ANSI 和 ASCII）
...
Global CompatibleNumber$ = "1.3.11-2022.3" ;当存档与构建版本不兼容时再更改		——开发者 ENDSHN
Global SinicizationNumber$ = "2022.4-First Phase Testing" ;汉化版本
...
AppTitle "SCP - 收容失效汉化版 v"+VersionNumber+" - 一期测试"
...
InitAAFont()
;因为某些问题，Blitz3D不会读取文件名与内部名不同的字体文件
;（内部名就是类似在Word里显示的字体名称，但这里一般推荐使用字体英文名）
;因此，我将他们重新命名，这样它们就不需要用FastText加载了			——开发者 Regalis

;这个问题在Blitz3D原版确实有
;但是不是用SpeedText也有这个问题就有待考证了		——子悦
...
Font4 = TextLoadFont("DigtalVonwaon", Int(60 * (GraphicHeight / 1024.0)), False, False, False, TEXT_DEFAULT, "GFX\font\DS-DIGI\DigtalVonwaon.ttf")
Font5 = TextLoadFont("Journal", Int(60 * (GraphicHeight / 1024.0)), False, False, False, TEXT_DEFAULT, "GFX\font\Journal\Journal.ttf")

ConsoleFont = TextLoadFont("Blitz", Int(20 * (GraphicHeight / 1024.0)), False, False, False, TEXT_DEFAULT, "")
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower(StrTemp)
				Case "help"
					...
					Select Lower(StrTemp)
						Case "1",""
							...
							CreateConsoleMsg("******************************")
							CreateConsoleMsg("使用 "+Chr(34)+"help 2"+Chr(34)+"或"+Chr(34)+"help 3"+Chr(34)+" 以查看更多命令")
							CreateConsoleMsg("使用 "+Chr(34)+"help [命令名]"+Chr(34)+" 以获得关于命令的详细信息")
							CreateConsoleMsg("******************************")
						Case "2"
						...
						Case "camerafog"
							...
							CreateConsoleMsg("******************************")
							CreateConsoleMsg("设置雾的绘制距离,迷雾在"+Chr(34)+"可视距离"+Chr(34)+"开始渲染,")
							CreateConsoleMsg("直到"+Chr(34)+"渲染距离"+Chr(34)+"处变得完全不透明")
						...
						Case "spawnitem"
							...
							CreateConsoleMsg("在汉化版本中，由于无法输入中文")
							CreateConsoleMsg("因此在汉化版中只能输入物品ID生成物品")
							CreateConsoleMsg("示例: spawnitem key6 (万能钥匙卡)")
							CreateConsoleMsg("******************************")
						Case "spawn"
				...
				Case "status"
					...
					For ev.Events = Each Events
						If ev\room = PlayerRoom Then
							...
							CreateConsoleMsg("-    状态: "+ev\EventState)
							CreateConsoleMsg("-    状态2: "+ev\EventState2)	
							CreateConsoleMsg("-    状态3: "+ev\EventState3)
							Exit
						EndIf
					Next
				...
				Case "scp-420-j","420","weed"
					...
					For i = 1 To 20
						If Rand(2)=1 Then
							it.Items = CreateItem("一些SCP-420-J","420", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
						Else
							it.Items = CreateItem("烟卷","420s", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5))
						EndIf
						EntityType (it\collider, HIT_ITEM)
					Next
				...
				Case "spawnradio"
					;[Block]
					it.Items = CreateItem("对讲机", "fineradio", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider))
					EntityType(it\collider, HIT_ITEM)
					it\state = 101
					;[End Block]
				Case "spawnnvg"
					;[Block]
					it.Items = CreateItem("夜视仪","nvgoggles", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider))
					EntityType(it\collider, HIT_ITEM)
					it\state = 1000
					;[End Block]
				...
				Case "spawnnav"
					;[Block]
					it.Items = CreateItem("S-NAV终极导航仪", "nav", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider))
					EntityType(it\collider, HIT_ITEM)
					it\state = 101
					;[End Block]
				Case "teleport173"
...
Repeat
	...
	Else
		...
		If EnableSubtitle Then UpdateSubtitles() ;字幕系统
		UpdateConsole()
		...
		Color 255, 255, 255
		If ShowFPS Then AASetFont ConsoleFont : Text 20, 20, "FPS: " + FPS : AASetFont Font1
			...
			If e\EventState = 0 And e\EventStr <> ""
				...
				ElseIf e\EventStr = "load2"
					...
					If e\room\NPC[0]=Null Then
						...
						Else
							DebugLog "跳过了room2sl生成scp-049的事件"
						EndIf
					EndIf
					QuickLoadPercent = 80
					e\EventStr = "load3"
...
Function DrawCredits()
    FontCredits1 = TextLoadFont("Containment Breach", 21, False, False, False, TEXT_DEFAULT, "GFX\font\cour\Containment Breach.ttf")
	FontCredits2 = TextLoadFont("Containment Breach", 35, True, False, False, TEXT_DEFAULT, "GFX\font\courbd\Containment Breach Bold.ttf")
	FontCredits3 = TextLoadFont("Containment Breach", 37, True, False, False, TEXT_DEFAULT, "GFX\font\courbd\Containment Breach Bold.ttf")
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		If Int(EntityX(Collider))=Int(Nan1) Then
			...
			Msg = "EntityX(碰撞) = 无， 正在重置位置    -    新位置: "+EntityX(Collider)
			MsgTimer = 300				
		EndIf
		;EndIf
		...
		If DebugHUD Then
			...
			For ev.Events = Each Events
				If ev\room = PlayerRoom Then
					...
					Text x - 50, 190, "状态: " + ev\EventState
					Text x - 50, 210, "状态2: " + ev\EventState2   
					Text x - 50, 230, "状态3: " + ev\EventState3
			...
			If Curr173 <> Null
				Text x - 50, 410, "SCP - 173 位置 (碰撞): (" + f2s(EntityX(Curr173\Collider), 3) + ", " + f2s(EntityY(Curr173\Collider), 3) + ", " + f2s(EntityZ(Curr173\Collider), 3) + ")"
				Text x - 50, 430, "SCP - 173 位置 (obj): (" + f2s(EntityX(Curr173\obj), 3) + ", " + f2s(EntityY(Curr173\obj), 3) + ", " + f2s(EntityZ(Curr173\obj), 3) + ")"
				;Text x - 50, 410, "SCP - 173 Idle: " + Curr173\Idle
				Text x - 50, 450, "SCP - 173 状态: " + Curr173\State
			EndIf
			If Curr106 <> Null
				Text x - 50, 470, "SCP - 106 位置: (" + f2s(EntityX(Curr106\obj), 3) + ", " + f2s(EntityY(Curr106\obj), 3) + ", " + f2s(EntityZ(Curr106\obj), 3) + ")"
				Text x - 50, 490, "SCP - 106 游荡: " + Curr106\Idle
				Text x - 50, 510, "SCP - 106 状态: " + Curr106\State
			EndIf
			offset% = 0
			For npc.NPCs = Each NPCs
				If npc\NPCtype = NPCtype096 Then
					Text x - 50, 530, "SCP - 096 位置: (" + f2s(EntityX(npc\obj), 3) + ", " + f2s(EntityY(npc\obj), 3) + ", " + f2s(EntityZ(npc\obj), 3) + ")"
					Text x - 50, 550, "SCP - 096 游荡: " + npc\Idle
					Text x - 50, 570, "SCP - 096 状态: " + npc\State
					Text x - 50, 590, "SCP - 096 速度: " + f2s(npc\currspeed, 5)
				EndIf
			...
			Text x + 350, 150, "SCP-427 状态(秒): "+Int(I_427\Timer/70.0)
			Text x + 350, 170, "SCP-008 感染: "+Infect
			For i = 0 To 5
				Text x + 350, 190+(20*i), "SCP-1025 状态 "+i+": "+SCP1025state[i]
			Next
						...
						Select SelectedItem\itemtemplate\tempname
							Case "paper","key1","key2","key3","key4","key5","key6","misc","oldpaper","badge","ticket","25ct","coin","key","scp860"
								...
								If Inventory(MouseSlot)\itemtemplate\tempname = "clipboard" Then
									...
									If (b<>"misc" And b<>"25ct" And b<>"coin" And b<>"key" And b<>"scp860" And b<>"scp714") Or (b2="扑克牌" Or b2="万事达信用卡") Then
								...
								ElseIf Inventory(MouseSlot)\itemtemplate\tempname = "wallet" Then
									...
									If (b<>"misc" And b<>"paper" And b<>"oldpaper") Or (b2="扑克牌" Or b2="万事达信用卡") Then
										For c% = 0 To Inventory(MouseSlot)\invSlots-1
											If (Inventory(MouseSlot)\SecondInv[c] = Null)
												If SelectedItem <> Null Then
													Inventory(MouseSlot)\SecondInv[c] = SelectedItem
							...
							Case "battery", "bat"
								...
								Select Inventory(MouseSlot)\itemtemplate\name
									Case "S-NAV Navigator", "S-NAV 300导航仪", "S-NAV 310导航仪"
										If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(PickSFX(SelectedItem\itemtemplate\sound))	
				...
				Case "杯子"
					...
					If CanUseItem(False,False,True)
						...
						If strtemp <> "" Then
							...
							MsgTimer = 70*6		
						Else
							it.Items = CreateItem("空杯子", "emptycup", 0,0,0)
							it\Picked = True
							For i = 0 To MaxItemAmount-1
				...
				Case "420s"
					...
					If CanUseItem(False,False,True)
						...
						Else
							...
							Msg = Chr(34) + "“我操我在哪...我在干什么...我需要小睡一会...”"
							KillTimer = -1						
						EndIf
						MsgTimer = 70 * 6
						RemoveItem(SelectedItem)
				...
				Case "navigator", "nav"
					...
					Else
						...
						If SelectedItem\state > 0 And (Rnd(CoffinDistance + 15.0) > 1.0 Or PlayerRoom\RoomTemplate\Name <> "coffin") Then
							...
							EndIf
							If (MilliSecs2() Mod 1000) > 300 Then
								If SelectedItem\itemtemplate\name <> "S-NAV 310导航仪" And SelectedItem\itemtemplate\name <> "S-NAV终极导航仪" Then
							...
							Local SCPs_found% = 0
							If SelectedItem\itemtemplate\name = "S-NAV终极导航仪" And (MilliSecs2() Mod 600) < 400 Then
...
Function DrawMenu()
	...
	If api_GetFocus() = 0 Then ;游戏失去焦点 -> 暂停游戏
	...
	If MenuOpen Then
		...
		If AchievementsMenu > 0 Then
			AASetFont Font2
			Text(x, y-(122-45)*MenuScale, "成就",False,True)
			AASetFont Font1
		ElseIf OptionsMenu > 0 Then
			AASetFont Font2
			Text(x, y-(122-45)*MenuScale, "设置",False,True)
			AASetFont Font1
		ElseIf QuitMSG > 0 Then
			AASetFont Font2
			Text(x, y-(122-45)*MenuScale, "退出？",False,True)
			AASetFont Font1
		ElseIf KillTimer >= 0 Then
			AASetFont Font2
			Text(x, y-(122-45)*MenuScale, "已暂停",False,True)
			AASetFont Font1
		Else
			AASetFont Font2
			Text(x, y-(122-45)*MenuScale, "你死了",False,True)
			AASetFont Font1
		End If		
		...
		If AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG <= 0
			...
			Text x, y+20*MenuScale, "存档名称: "+CurrSave
			Text x, y+40*MenuScale, "地图种子: "+RandomSeed
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			If DrawButton(x + 101 * MenuScale, y + 390 * MenuScale, 230 * MenuScale, 60 * MenuScale, "返回") Then
				AchievementsMenu = 0
				OptionsMenu = 0
				QuitMSG = 0
				MouseHit1 = False
			...
			Select OptionsMenu
				...
				Case 2 ;Audio
					...
					If EnableUserTracks
						...
						If UserTrackMode
							Text x, y + 20 * MenuScale, "循环"
						Else
							Text x, y + 20 * MenuScale, "随机"
						EndIf
						If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
				...
				Case 3 ;Controls
					...
					Text(x, y + 140 * MenuScale, "打开/关闭物品栏")
					...
					Text(x, y + 200 * MenuScale, "打开/关闭控制台")
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG > 0 And KillTimer >= 0
			...
			If SelectedDifficulty\saveType = SAVEONQUIT Or SelectedDifficulty\saveType = SAVEANYWHERE Then
				...
				If AbleToSave
					QuitButton = 140
					If DrawButton(x, y + 60*MenuScale, 390*MenuScale, 60*MenuScale, "保存并退出") Then
						DropSpeed = 0
						SaveGame(SavePath + CurrSave + "\")
						NullGame()
						MenuOpen = False
			...
			If DrawButton(x, y + QuitButton*MenuScale, 390*MenuScale, 60*MenuScale, "退出") Then
				NullGame()
				MenuOpen = False
				MainMenuOpen = True
				MainMenuTab = 0
			...
			If DrawButton(x+101*MenuScale, y + 344*MenuScale, 230*MenuScale, 60*MenuScale, "返回") Then
				AchievementsMenu = 0
				OptionsMenu = 0
				QuitMSG = 0
				MouseHit1 = False
		...
		If AchievementsMenu<=0 And OptionsMenu<=0 And QuitMSG<=0 Then
			If KillTimer >= 0 Then	
				...
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "继续", True, True) Then
					MenuOpen = False
					ResumeSounds()
					MouseXSpeed() : MouseYSpeed() : MouseZSpeed() : mouse_x_speed_1#=0.0 : mouse_y_speed_1#=0.0
				EndIf
				...
				If (Not SelectedDifficulty\permaDeath) Then
					If GameSaved Then
						If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "加载游戏") Then
							DrawLoading(0)
					...
					Else
						...
						Color (100, 100, 100)
						AASetFont Font2
						Text(x + (390*MenuScale) / 2, y + (60*MenuScale) / 2, "加载游戏", True, True)
					EndIf
					y = y + 75*MenuScale
			EndIf
				
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "成就") Then AchievementsMenu = 1
				y = y + 75*MenuScale
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "设置") Then OptionsMenu = 1
				y = y + 75*MenuScale
			Else
				y = y+104*MenuScale
				...
				EndIf
				If DrawButton(x, y + 80*MenuScale, 390*MenuScale, 60*MenuScale, "退回主菜单") Then
					NullGame()
					MenuOpen = False
					MainMenuOpen = True
					MainMenuTab = 0
			...
			If KillTimer >= 0 And (Not MainMenuOpen)
				If DrawButton(x, y, 390*MenuScale, 60*MenuScale, "退出") Then
					QuitMSG = 1
				EndIf
			EndIf
...
Function LoadEntities()
	...
	LiquidObj = LoadMesh_Strict("GFX\items\cupliquid.x") ;optimized the 杯子s dispensed by 294
...
;	HideEntity OBJTunnel(6)
	...
	If EnableUserTracks Then
		...
		CloseDir Dir
	EndIf
	If EnableUserTracks Then DebugLog "自定频道找到: "+UserTrackMusicAmount
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (r\RoomTemplate\Name = "start" And IntroEnabled = False) Then 
			...
			PlayerRoom = r
			it = CreateItem("D级人员培训传单", "paper", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			...
			EntityParent(it\collider, 0)
			ItemAmount = ItemAmount + 1
			it = CreateItem("SCP-173文档", "paper", 1, 1, 1)
			it\Picked = True
			it\Dropped = -1
			it\itemtemplate\found=True
			Inventory(1) = it
	...
	Select item\itemtemplate\name
		Case "防毒面具", "重型防毒面具"
			Select setting
				...
				Case "fine", "very fine"
					it2 = CreateItem("防毒面具", "supergasmask", x, y, z)
					RemoveItem(item)
			End Select
		Case "SCP-1499"
				Select setting
				Case "rough", "coarse"
					...
					RemoveItem(item)
				Case "1:1"
					it2 = CreateItem("防毒面具", "gasmask", x, y, z)
					RemoveItem(item)
				Case "fine"
		...
		Case "防弹背心"
			Select setting
				...
				Case "1:1"
					...
					ResetEntity(item\collider)
				Case "fine"
					it2 = CreateItem("重型防弹背心", "finevest", x, y, z)
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("笨重的防弹背心", "veryfinevest", x, y, z)
					RemoveItem(item)
			End Select
		Case "文件夹"
			Select setting
		...
		Case "夜视仪"
			Select setting
				...
				Case "1:1"
					...
					ResetEntity(item\collider)
				Case "fine"
					it2 = CreateItem("夜视仪", "finenvgoggles", x, y, z)
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("夜视仪", "supernv", x, y, z)
					it2\state = 1000
					RemoveItem(item)
			End Select
		Case "金属板", "心灵遮断合金锭"
			Select setting
				Case "rough", "coarse"
					it2 = CreateItem("心灵遮断合金锭", "scp148ingot", x, y, z)
					RemoveItem(item)
				Case "1:1", "fine", "very fine"
					it2 = Null
					For it.Items = Each Items
					...
					If it2<>Null Then
						Select it2\itemtemplate\tempname
							Case "gasmask", "supergasmask"
								...
								it2 = CreateItem("重型防毒面具", "gasmask3", x, y, z)
							Case "vest"
								RemoveItem (it2)
								RemoveItem(item)
								it2 = CreateItem("重型防弹背心", "finevest", x, y, z)
							Case "hazmatsuit","hazmatsuit2"
								RemoveItem (it2)
								RemoveItem(item)
								it2 = CreateItem("重型防护服", "hazmatsuit3", x, y, z)
						End Select
					Else 
						If item\itemtemplate\name="心灵遮断合金锭" Then
							it2 = CreateItem("金属板", "scp148", x, y, z)
							RemoveItem(item)
						Else
							PositionEntity(item\collider, x, y, z)
							ResetEntity(item\collider)							
		...
		Case "断手", "黑色断手"
			Select setting
				...
				Case "1:1", "fine", "very fine"
					If (item\itemtemplate\name = "断手")
						it2 = CreateItem("黑色断手", "hand2", x, y, z)
					Else
						it2 = CreateItem("断手", "hand", x, y, z)
					EndIf
			End Select
			RemoveItem(item)
		Case "急救箱", "蓝色急救箱"
			Select setting
				...
				Case "1:1"
				If Rand(2)=1 Then
					it2 = CreateItem("蓝色急救箱", "firstaid2", x, y, z)
				Else
				    it2 = CreateItem("急救箱", "firstaid", x, y, z)
				EndIf
				Case "fine"
					it2 = CreateItem("小型急救箱", "finefirstaid", x, y, z)
				Case "very fine"
					it2 = CreateItem("奇怪的瓶子", "veryfinefirstaid", x, y, z)
			End Select
			RemoveItem(item)
		Case "一级钥匙卡", "二级钥匙卡", "三级钥匙卡", "四级钥匙卡", "五级钥匙卡", "钥匙卡"
			Select setting
				Case "rough", "coarse" 
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("扑克牌", "misc", x, y, z)
				Case "fine"
					Select item\itemtemplate\name
						Case "一级钥匙卡"
							Select SelectedDifficulty\otherFactors
								Case EASY
									it2 = CreateItem("二级钥匙卡", "key2", x, y, z)
								Case NORMAL
									If Rand(5)=1 Then
										it2 = CreateItem("万事达信用卡", "misc", x, y, z)
									Else
										it2 = CreateItem("二级钥匙卡", "key2", x, y, z)
									EndIf
								Case HARD
						...
						Case "二级钥匙卡"
							Select SelectedDifficulty\otherFactors
								Case EASY
									it2 = CreateItem("三级钥匙卡", "key3", x, y, z)
								Case NORMAL
									If Rand(4)=1 Then
										it2 = CreateItem("万事达信用卡", "misc", x, y, z)
									Else
						...
						Case "三级钥匙卡"
							Select SelectedDifficulty\otherFactors
								Case EASY
									If Rand(10)=1 Then
										it2 = CreateItem("四级钥匙卡", "key4", x, y, z)
									Else
										it2 = CreateItem("扑克牌", "misc", x, y, z)	
									EndIf
								Case NORMAL
						...
						Case "四级钥匙卡"
							Select SelectedDifficulty\otherFactors
								Case EASY
									it2 = CreateItem("五级钥匙卡", "key5", x, y, z)
								Case NORMAL
									If Rand(4)=1 Then
										it2 = CreateItem("万事达信用卡", "misc", x, y, z)
									Else
							...
							Select SelectedDifficulty\otherFactors
								Case EASY
									If Rand(0,((MAXACHIEVEMENTS-1)*3)-((CurrAchvAmount-1)*3))=0
										it2 = CreateItem("万能钥匙卡", "key6", x, y, z)
									Else
										it2 = CreateItem("万事达信用卡", "misc", x, y, z)
									EndIf
								Case NORMAL
		...
		Case "扑克牌", "硬币", "硬币"
			Select setting
				Case "rough", "coarse"
					...
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("一级钥匙卡", "key1", x, y, z)	
			    Case "fine", "very fine"
					it2 = CreateItem("二级钥匙卡", "key2", x, y, z)
			End Select
			RemoveItem(item)
		Case "万事达信用卡"
			Select setting
				Case "rough"
					...
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "coarse"
					it2 = CreateItem("硬币", "25ct", x, y, z)
					Local it3.Items,it4.Items,it5.Items
					it3 = CreateItem("硬币", "25ct", x, y, z)
					it4 = CreateItem("硬币", "25ct", x, y, z)
					it5 = CreateItem("硬币", "25ct", x, y, z)
			...
			End Select
			RemoveItem(item)
		Case "S-NAV 300导航仪", "S-NAV 310导航仪", "S-NAV Navigator", "S-NAV终极导航仪"
			Select setting
				Case "rough", "coarse"
					it2 = CreateItem("电子元件", "misc", x, y, z)
				Case "1:1"
					it2 = CreateItem("S-NAV Navigator", "nav", x, y, z)
					it2\state = 100
				Case "fine"
					it2 = CreateItem("S-NAV 310导航仪", "nav", x, y, z)
					it2\state = 100
				Case "very fine"
					it2 = CreateItem("S-NAV终极导航仪", "nav", x, y, z)
					it2\state = 101
			End Select
		...
		Case "对讲机"
			Select setting
				Case "rough", "coarse"
					it2 = CreateItem("电子元件", "misc", x, y, z)
				Case "1:1"
					it2 = CreateItem("对讲机", "18vradio", x, y, z)
					it2\state = 100
				Case "fine"
					it2 = CreateItem("对讲机", "fineradio", x, y, z)
					it2\state = 101
				Case "very fine"
					it2 = CreateItem("对讲机", "veryfineradio", x, y, z)
					it2\state = 101
			End Select
		...
		Case "一些SCP-420-J", "香烟"
			Select setting
				Case "rough", "coarse"			
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("香烟", "cigarette", x + 1.5, y + 0.5, z + 1.0)
				Case "fine"
					it2 = CreateItem("烟卷", "420s", x + 1.5, y + 0.5, z + 1.0)
				Case "very fine"
					it2 = CreateItem("难闻的烟卷", "420s", x + 1.5, y + 0.5, z + 1.0)
			End Select
			
			RemoveItem(item)
		Case "9V电池", "18V电池", "奇怪的电池"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("18V电池", "18vbat", x, y, z)
				Case "fine"
					it2 = CreateItem("奇怪的电池", "killbat", x, y, z)
				Case "very fine"
					it2 = CreateItem("奇怪的电池", "killbat", x, y, z)
			End Select
			
			RemoveItem(item)
		Case "ReVision眼药水", "RedVision眼药水", "眼药水"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("RedVision眼药水", "eyedrops", x,y,z)
				Case "fine"
					it2 = CreateItem("眼药水", "fineeyedrops", x,y,z)
				Case "very fine"
					it2 = CreateItem("眼药水", "supereyedrops", x,y,z)
			End Select
		...
		Case "防护服"
			Select setting
				Case "rough", "coarse"
					...
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("防护服", "hazmatsuit", x,y,z)
				Case "fine"
					it2 = CreateItem("防护服", "hazmatsuit2", x,y,z)
				Case "very fine"
					it2 = CreateItem("防护服", "hazmatsuit2", x,y,z)
			End Select
				...
				Case "syringe"
					Select setting
						...
						Case "1:1"
							it2 = CreateItem("小型急救箱", "finefirstaid", x, y, z)	
						Case "fine"
							it2 = CreateItem("针筒", "finesyringe", x, y, z)
						Case "very fine"
							it2 = CreateItem("针筒", "veryfinesyringe", x, y, z)
					End Select
		...
		Case "SCP-500-01", "升级药丸", "药丸"
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("药丸", "pill", x, y, z)
					RemoveItem(item)
				Case "fine"
					...
					Else
						it2 = CreateItem("升级药丸", "scp500death", x, y, z)
					EndIf
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("升级药丸", "scp500death", x, y, z)
				...
				Case "paper"
					Select setting
						...
						Case "1:1"
							Select Rand(6)
								Case 1
									it2 = CreateItem("SCP-106文档", "paper", x, y, z)
								Case 2
									it2 = CreateItem("SCP-079文档", "paper", x, y, z)
								Case 3
									it2 = CreateItem("SCP-173文档", "paper", x, y, z)
								Case 4
									it2 = CreateItem("SCP-895文档", "paper", x, y, z)
								Case 5
									it2 = CreateItem("SCP-682文档", "paper", x, y, z)
								Case 6
									it2 = CreateItem("SCP-860文档", "paper", x, y, z)
							End Select
						Case "fine", "very fine"
							it2 = CreateItem("千纸鹤", "misc", x, y, z)
					End Select
	...
	If temp Then
		If MouseHit1 Then
			...
			If temp And Input294<>"" Then ;dispense
				...
				If Left(Input294, Min(7,Len(Input294))) = "一杯" Then
					Input294 = Right(Input294, Len(Input294)-7)
				ElseIf Left(Input294, Min(9,Len(Input294))) = "一杯" 
				...
				If loc > 0 Then
					...
					it.items = CreateItem("杯子", "杯子", EntityX(PlayerRoom\Objects[1],True),EntityY(PlayerRoom\Objects[1],True),EntityZ(PlayerRoom\Objects[1],True), r,g,b,alpha)
					it\name = "一杯"+Input294
					EntityType (it\collider, HIT_ITEM)
...
Function SaveOptionsINI()
	...
	PutINIValue(OptionFile, "options", "subtitle", EnableSubtitle%)
