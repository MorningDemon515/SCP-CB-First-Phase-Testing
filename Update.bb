...
; 我不知道这个东西在汉化版里有啥用，因为我手上没有合适的服务器来检查是否更新
; 如果说可以的话，我可以试试用github托管的官网？
; ——子悦

Global UserAgent$ = "SCPCB"	; What ever you'd like to name your application
Global ResponseDelay% = 2		; How many millisecs to wait for HTTP response
...
Function OpenRemoteFile%(path$, port% = 443, variables$ = "", httpheader$ = "")
...
;Strip protocol and return false if not "http"
	inst = Instr(link$, ":/")
	If inst Then
		If Lower(Trim(Left(link$, inst - 1))) <> "https" Then Return False
			...
			ElseIf conlen > 0 Then
				...
				Repeat
					...
					If avail > DOWNLOAD_SIZE Then
						...
						If DrawButton2(270,400,100,20,"取消",False) Then
							CloseTCPStream(tcp)
							FreeBank bnk
							CloseFile file
							Return -1
...
Function CheckForUpdates()
	InitAAFont()
	UpdaterFont% = TextLoadFont("Containment Breach", 20, False, False, False, TEXT_DEFAULT, "GFX\font\cour\Containment Breach.ttf")
	SetFont UpdaterFont
	...
	;Local updateCheck% = OpenRemoteFile("https://scpcbgame.cn/changelog.txt")
	...
	;If latest<>VersionNumber And latest<>"" Then ;a new version is available!
		...
		;Local ChangeLogURL$ = "https:/scpcbgame.cn/changelog.txt"
